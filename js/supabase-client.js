...
(function(window) {
  ...
  function alcanceVentas29S_(scopeRows, user) {
    ...
    var alcance = String((found && found.alcance) || "GLOBAL").toUpperCase();
    ...
    var test = function(f) {
      f = f || {};
      if (alcance === "PROPIO") return String(f.idUsuario || "") === me;
      if (alcance === "PROVEEDOR" || alcance === "ASIGNADOS") {
        if (!mp) return true;
        return String(f.idProveedor || "") === mp;
      }
      if (alcance === "OFICINA") {
        if (!mo) return true;
        return String(f.idOficina || "") === mo;
      }
      if (alcance === "GRUPO") {
        if (!mg) return true;
        return String(f.idGrupo || "") === mg;
      }
      return true;
    };
  ...
  async function executeSupabaseOperation(client, operation, args, user) {
    ...
    if (operation === "listarVentasContadoModulo" || operation === "obtenerDetalleVentaContadoModulo") {
      ...
      try {
        ...
        if (!scopeQ29S_.error) scopeRows29S_=scopeQ29S_.data || [];
      } catch (_) {}
      var scope29S_=alcanceVentas29S_(scopeRows29S_, user);
      var visible29S_=mapped.filter(function(m) { try { return scope29S_.test(m); } catch (_) { return true; } });
      var bono29S_=0;
      try {
