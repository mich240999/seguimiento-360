...
(function(window) {
  ...
  async function loadContext() {
    ...
    const superadminResources = ["VISUALIZAR_MODULO","ADMINISTRAR","CREAR","EDITAR","ELIMINAR","IMPORTAR","EXPORTAR","VER_LISTADO","VER_MATERIALES","CREAR_MATERIAL","EDITAR_MATERIAL","GESTIONAR_CATALOGO","GESTIONAR_PRECIOS","CREAR_PRECIO_INDIVIDUAL","CREAR_LISTA_OFICIAL","EDITAR_LISTA_OFICIAL","REGISTRAR_VENTA","EDITAR_VENTA","CONFIRMAR_ABONO","PROGRAMAR_ENTREGA","CONFIRMAR_ENTREGA","ANULAR_VENTA","VER_PRECIOS","VER_LISTAS_OFICIALES","VER_MIS_LISTAS_PRECIO","VER_SOLICITUDES_PRECIO","CARGAR_LISTA_PRECIO","CARGAR_LISTA_PRECIO_ADMIN"];
    permitted.forEach(function(module) { context.permisos[module.codigo] = context.permisos[module.codigo] || {}; if (role === "SUPERADMIN") superadminResources.forEach(function(resource) { context.permisos[module.codigo][resource] = { permitido: true, alcance: "TOTAL" }; }); });
  ...
  async function executeSupabaseOperation(client, operation, args, user) {
    ...
    if (operation === "listarCatalogosAdminMotor") { const rows = await query("app_catalogos"); return rows.map(function(x) { return { idCatalogo:x.id_catalogo,codigo:x.codigo,nombre:x.nombre,descripcion:x.descripcion,estado:x.estado }; }); }
    if (operation === "listarRecursosVisualesAdminMotor") return [];
    if (operation === "listarRolesAdminMotor" || operation === "listarRolesAsignablesUsuariosMotor") { const rows = await query("seg_roles"); return { roles: rows.map(function(x) { return { idRol:x.id_rol,codigo:x.codigo,nombre:x.nombre,descripcion:x.descripcion,nivel:x.nivel,estado:x.estado,sistema:x.sistema }; }) }; }
    if (operation === "listarUsuariosAdminMotor") { const rows = await query("seg_usuarios"); return { registros: rows.map(function(x) { return { idUsuario:x.id_usuario, correo:x.correo, nombre:x.nombre, telefono:x.telefono, rol:x.rol, idProveedor:x.id_proveedor, idOficina:x.id_oficina, idGrupo:x.id_grupo, estado:x.estado, tipoDocumento:x.tipo_documento, numeroDocumento:x.numero_documento }; }), contextoAcceso: { idUsuario:user.id_usuario, rol:user.rol, alcance:"GLOBAL" } }; }
    if (operation === "guardarUsuarioAdminMotor") { const x=args[0] || {}; const id=x.idUsuario || "USR-"+Date.now(); const saved=await client.from("seg_usuarios").upsert({id_usuario:id,correo:String(x.correo || "").trim().toLowerCase(),nombre:x.nombre || "Usuario",telefono:x.telefono || null,rol:x.rol || "USUARIO",id_proveedor:x.idProveedor || null,id_oficina:x.idOficina || null,id_grupo:x.idGrupo || null,estado:x.estado || "ACTIVO",tipo_documento:x.tipoDocumento || "DNI",numero_documento:x.numeroDocumento || null,id_usuario_actualizacion:user.id_usuario}); if(saved.error) throw saved.error; return {correcto:true,idUsuario:id}; }
    if (operation === "obtenerEstructuraAsignacionesAdminMotor" || operation === "obtenerEstructuraAsignacionesUsuariosMotor") { const all = await Promise.all([query("mae_proveedores"),query("mae_oficinas"),query("mae_grupos")]); return { proveedores:all[0].map(function(x){return {idProveedor:x.id_proveedor,razonSocial:x.razon_social,nombreComercial:x.nombre_comercial,estado:x.estado};}), oficinas:all[1].map(function(x){return {idOficina:x.id_oficina,nombre:x.nombre,estado:x.estado};}), grupos:all[2].map(function(x){return {idGrupo:x.id_grupo,idOficina:x.id_oficina,nombre:x.nombre,estado:x.estado};}), relacionesProveedorOficina:[], tiposDocumento:[{codigo:"DNI",nombre:"DNI"},{codigo:"CE",nombre:"Carné de extranjería"}], reglas:{} }; }
    ...
    if (operation === "listarMaterialesPrecioModulo" || operation === "listarMaterialesSelectPreciosModulo") { const all=await Promise.all([query("mae_materiales"),query("mae_marcas")]); const records = all[0].map(function(x) { const brand=all[1].find(function(b){return b.id_marca===x.id_marca;}); return { idMaterial:x.id_material,codigoMaterial:x.codigo_material,codigoSap:x.codigo_sap,idMarca:x.id_marca,marca:brand&&brand.nombre,nombreMaterial:x.nombre_material,descripcionMaterial:x.descripcion_material,unidadMedida:x.unidad_medida,estado:x.estado }; }); return operation === "listarMaterialesSelectPreciosModulo" ? { registros: records } : normalizeModuleResponse(operation, { materiales: records }); }
    if (operation === "obtenerOpcionesMaterialesPreciosModulo") { const all=await Promise.all([query("mae_proveedores"),query("mae_oficinas"),query("mae_grupos"),query("mae_materiales"),query("mae_marcas"),query("mae_negocios"),query("mae_productos_principales"),query("mae_tipos_material"),query("mae_subtipos_material")]); return { negocios:all[5].map(function(x){return {id:x.id_negocio,nombre:x.nombre};}), productos:all[6].map(function(x){return {id:x.id_producto,idNegocio:x.id_negocio,nombre:x.nombre};}), tipos:all[7].map(function(x){return {id:x.id_tipo_material,nombre:x.nombre};}), subtipos:all[8].map(function(x){return {id:x.id_subtipo_material,idProducto:x.id_producto,idTipoMaterial:x.id_tipo_material,nombre:x.nombre};}), marcas:all[4].map(function(x){return {id:x.id_marca,nombre:x.nombre};}), unidades:[{id:"UND",nombre:"Unidad"}], proveedores:all[0].map(function(x){return {idProveedor:x.id_proveedor,nombre:x.nombre_comercial || x.razon_social};}), oficinas:all[1].map(function(x){return {idOficina:x.id_oficina,nombre:x.nombre};}), grupos:all[2].map(function(x){return {idGrupo:x.id_grupo,idOficina:x.id_oficina,nombre:x.nombre};}), materiales:all[3].map(function(x){return {idMaterial:x.id_material,codigoMaterial:x.codigo_material,nombreMaterial:x.nombre_material};}) }; }
    if (operation === "obtenerPlantillaMaterialesModulo") return file("plantilla_materiales.csv",["CODIGO_SAP","NOMBRE_MATERIAL","DESCRIPCION_MATERIAL","MARCA","UNIDAD_MEDIDA","ESTADO"],[["SAP-001","Material ejemplo","Descripción del material","Marca ejemplo","UND","ACTIVO"]]);
    if (operation === "obtenerPlantillaPreciosIndividualesModulo" || operation === "obtenerPlantillaListaPreciosModulo") return file("plantilla_precios.csv",["CODIGO_SAP","CODIGO_MATERIAL","PRECIO_BASE","MONEDA","FECHA_INICIO","FECHA_FIN","DETALLE_COMBO"],[["SAP-001","MAT-001","100.00","PEN","2026-01-01","","Opcional"]]);
    ...
    if (operation === "obtenerContextoVentasContadoModulo") { const context = await loadContext(); const p = context.permisos.VENTAS_CONTADO || {}; return { permisos:{ puedeListar:!!(p.VISUALIZAR_MODULO && p.VISUALIZAR_MODULO.permitido), puedeRegistrar:!!(p.REGISTRAR_VENTA && p.REGISTRAR_VENTA.permitido), puedeEditar:!!(p.EDITAR_VENTA && p.EDITAR_VENTA.permitido), puedeValidarAbono:!!(p.CONFIRMAR_ABONO && p.CONFIRMAR_ABONO.permitido), puedeGestionarEntrega:!!(p.PROGRAMAR_ENTREGA && p.PROGRAMAR_ENTREGA.permitido), puedeExportar:!!(p.EXPORTAR && p.EXPORTAR.permitido) }, vistaInicial:"REGISTRADAS", estados:[], estadosAbono:[], negociosVenta:[] }; }
    return undefined;
  }
