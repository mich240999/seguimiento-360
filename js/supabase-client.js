...
(function(window) {
  ...
  async function executeSupabaseOperation(client, operation, args, user) {
    ...
    if (operation === "guardarProveedorModulo") { const x=args[0] || {}; const id=x.idProveedor || "PRV-"+Date.now(); const saved=await client.from("mae_proveedores").upsert({id_proveedor:id,razon_social:x.razonSocial || x.nombreComercial || "Proveedor",nombre_comercial:x.nombreComercial || x.razonSocial || "Proveedor",codigo_sap:x.codigoSap || null,ruc:x.ruc || null,alcance_catalogo:x.alcanceCatalogo || "TOTAL",estado:x.estado || "ACTIVO"}); if(saved.error) throw saved.error; return {correcto:true,idProveedor:id,nombreComercial:x.nombreComercial || x.razonSocial}; }
    if (operation === "guardarMaterialPrecioModulo") { const x=args[0] || {}; const id=x.idMaterial || "MAT-"+Date.now(); const saved=await client.from("mae_materiales").upsert({id_material:id,codigo_material:x.codigoMaterial || id,codigo_sap:x.codigoSap || null,nombre_material:x.nombreMaterial || x.descripcionMaterial || "Material",descripcion_material:x.descripcionMaterial || null,unidad_medida:x.unidadMedida || "UND",es_gasodomestico:!!x.esGasodomestico,estado:x.estado || "ACTIVO"}); if(saved.error) throw saved.error; return {correcto:true,idMaterial:id,mensaje:"Material guardado correctamente."}; }
    if (operation === "listarProveedoresModulo") { const rows = await query("mae_proveedores"); const records = rows.map(function(x) { return { idProveedor:x.id_proveedor,razonSocial:x.razon_social,nombreComercial:x.nombre_comercial,codigoSap:x.codigo_sap,ruc:x.ruc,alcanceCatalogo:x.alcance_catalogo,estado:x.estado }; }); return normalizeModuleResponse(operation, { proveedores: records }); }
    if (operation === "listarMaterialesPrecioModulo" || operation === "listarMaterialesSelectPreciosModulo") { const rows = await query("mae_materiales"); const records = rows.map(function(x) { return { idMaterial:x.id_material,codigoMaterial:x.codigo_material,codigoSap:x.codigo_sap,nombreMaterial:x.nombre_material,descripcionMaterial:x.descripcion_material,estado:x.estado }; }); return operation === "listarMaterialesSelectPreciosModulo" ? { registros: records } : normalizeModuleResponse(operation, { materiales: records }); }
    if (operation === "obtenerOpcionesMaterialesPreciosModulo") { const [proveedores, oficinas, grupos, materiales] = await Promise.all([query("mae_proveedores"), query("mae_oficinas"), query("mae_grupos"), query("mae_materiales")]); return { negocios:[], productos:[], tipos:[], subtipos:[], marcas:[], unidades:[], proveedores:proveedores.map(function(x){return {idProveedor:x.id_proveedor,nombre:x.nombre_comercial || x.razon_social};}), oficinas:oficinas.map(function(x){return {idOficina:x.id_oficina,nombre:x.nombre};}), grupos:grupos.map(function(x){return {idGrupo:x.id_grupo,idOficina:x.id_oficina,nombre:x.nombre};}), materiales:materiales.map(function(x){return {idMaterial:x.id_material,codigoMaterial:x.codigo_material,nombreMaterial:x.nombre_material};}) }; }
