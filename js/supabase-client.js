...
(function(window) {
  ...
  async function registrarEventoSupabase_(client, user, operation, args, moduleCode) {
    ...
    const id = input.idUsuario || input.idSesion || input.idMaterial || input.idProveedor || input.idOficina || input.idGrupo || input.idVenta || input.idValor || input.idDetallePrecio || input.idEmpresa || "";
  ...
  async function ensureProveedorChain_(client, user, input, idCanal) {
    input = input || {};
    var warnings = [];
    var byUser = (user && user.id_usuario) || null;
    var canalOrigen_ = String(idCanal != null ? idCanal : (input.idCanal || input.canal || "")).trim();
    ...
    if (!providerRow && providerText) {
      ...
      if (!providerRow) {
        ...
        var provPayload_ = { id_proveedor: newProvId, razon_social: providerText, nombre_comercial: providerText, nombre: providerText, alcance_catalogo: "TOTAL", estado: "ACTIVO", id_usuario_actualizacion: byUser };
        try {
          var canalNorm_ = String(canalOrigen_ || "").trim().toUpperCase();
          if (canalNorm_ === "IA") canalNorm_ = "CANAL-IA";
          else if (canalNorm_ === "ALO") canalNorm_ = "CANAL-ALO";
          if (canalNorm_) provPayload_.id_canal_origen = canalNorm_;
        } catch (_) {}
        var createdProv = await client.from("mae_proveedores").insert(provPayload_);
        if (createdProv.error && String((createdProv.error && createdProv.error.message) || "").indexOf("id_canal_origen") !== -1) {
          try { delete provPayload_.id_canal_origen; } catch (_) {}
          createdProv = await client.from("mae_proveedores").insert(provPayload_);
        }
    ...
    // 2026-09-23: autocreacion de catalogos LP-CAT desactivada. El catalogo ahora ES la lista del canal (una lista por canal con todos los materiales); cada detalle indica su proveedor via id_proveedor. Se conserva proveedor/oficina/grupo del chain.
    if (false && providerRow && providerRow.id_proveedor) {
      // AGENTE A (2026-09-22): cada proveedor tiene UN catálogo (es_catalogo). [DESACTIVADO 2026-09-23: ver comentario anterior]
  ...
  // 2026-09-23: derivacion de proveedor por detalle. Prioridad (1) detalle.id_proveedor, (2) lista.id_proveedor, (3) match material+precio. Defensivo: si la columna no existe, vuelve al comportamiento anterior.
  async function proveedoresDetalleVenta29S_(client, idVenta) {
    ...
    var pd = null;
    try {
      pd = await client.from("pre_lista_precio_detalle").select("id_material,precio_base,id_lista_precio,id_proveedor").in("id_material", mats);
      if (pd.error && String((pd.error && pd.error.message) || "").indexOf("id_proveedor") !== -1) {
        pd = await client.from("pre_lista_precio_detalle").select("id_material,precio_base,id_lista_precio").in("id_material", mats);
      }
    } catch (_) {
      pd = await client.from("pre_lista_precio_detalle").select("id_material,precio_base,id_lista_precio").in("id_material", mats);
    }
    ...
    rows.forEach(function(d) {
      var hits = (pd.data || []).filter(function(p) { return String(p.id_material || "") === String(d.id_material || "") && Number(p.precio_base || 0) === Number(d.precio_unitario || 0); });
    ...
    var mapListaProv = {};
    (lp.data || []).forEach(function(l) { mapListaProv[String(l.id_lista_precio || "")] = String(l.id_proveedor || ""); });
    var out = [], seenP = {};
    var pushProv = function(p0) { p0 = String(p0 || "").trim(); if (p0 && !seenP[p0]) { seenP[p0] = true; out.push(p0); } };
    rows.forEach(function(d) {
      var hits2 = (pd.data || []).filter(function(p) { return String(p.id_material || "") === String(d.id_material || "") && Number(p.precio_base || 0) === Number(d.precio_unitario || 0); });
      hits2.forEach(function(hit) {
        var provDet = String((hit && hit.id_proveedor) || "").trim();
        if (provDet) { pushProv(provDet); return; }
        var provLista = mapListaProv[String((hit && hit.id_lista_precio) || "")] || "";
        if (provLista) pushProv(provLista);
      });
    });
    return out;
  }
  ...
  async function executeSupabaseOperation(client, operation, args, user) {
    ...
    if (operation === "listarVentasContadoModulo" || operation === "obtenerDetalleVentaContadoModulo") {
      ...
      const map=function(s){
        const rows=(detailRows.data||[]).filter(function(d){return d.id_venta===s.id_venta;}).map(function(d){
          ...
          var provEfRow_=""; try { provEfRow_=String((price&&price.id_proveedor)||(list&&list.id_proveedor)||"").trim(); } catch (_) { try { provEfRow_=String((list&&list.id_proveedor)||"").trim(); } catch (_2) {} }
          const provider=related[3].find(function(x){return !!provEfRow_&&String(x.id_proveedor||"")===provEfRow_;})||related[3].find(function(x){return x.id_proveedor===list.id_proveedor;})||{};
          return {idDetalleVenta:d.id_detalle_venta,idDetallePrecio:price.id_detalle_precio||"",idListaPrecio:list.id_lista_precio||"",idMaterial:d.id_material,codigoMaterial:d.codigo_material||material.codigo_material||"",descripcionMaterial:d.descripcion_material||material.nombre_material||material.descripcion_material||"Material",cantidad:d.cantidad,precioUnitario:d.precio_unitario,totalLinea:d.precio_total,moneda:s.moneda,detalleCombo:d.detalle_combo,estadoItem:d.estado_material,idProveedorPrecio:provEfRow_||"",nombreComercialProveedor:provider.nombre_comercial||provider.razon_social||"",codigoSapProveedor:provider.codigo_sap||""};
        });
    ...
    if (operation === "cambiarEstadoEmpresaVendedoraAdminMotor") { const role29E_=String(user.rol||"").toUpperCase(); if(["SUPERADMIN","ADMIN"].indexOf(role29E_)===-1) throw new Error("Solo un administrador puede cambiar el estado."); const x29E_=args[0]||{}; const id29E_=String(x29E_.idEmpresa||"").trim(); const est29E_=String(x29E_.estado||"").toUpperCase()==="INACTIVO"?"INACTIVO":"ACTIVO"; if(!id29E_) throw new Error("Indica la empresa."); const saved29E_=await client.from("ven_empresas").update({estado:est29E_}).eq("id_empresa",id29E_); if(saved29E_.error) throw saved29E_.error; return {correcto:true,idEmpresa:id29E_,estado:est29E_,mensaje:"Empresa actualizada."}; }
    if (operation === "cambiarEstadoUsuarioAdminMotor") { const role29U_=String(user.rol||"").toUpperCase(); if(["SUPERADMIN","ADMIN"].indexOf(role29U_)===-1) throw new Error("Solo un administrador puede cambiar el estado."); const x29U_=args[0]||{}; const id29U_=String(x29U_.idUsuario||"").trim(); const est29U_=String(x29U_.estado||"").toUpperCase()==="INACTIVO"?"INACTIVO":"ACTIVO"; if(!id29U_) throw new Error("Indica el usuario."); if(id29U_===String(user.id_usuario||"")) throw new Error("No puedes desactivar tu propio usuario."); const saved29U_=await client.from("seg_usuarios").update({estado:est29U_,id_usuario_actualizacion:user.id_usuario}).eq("id_usuario",id29U_); if(saved29U_.error) throw saved29U_.error; return {correcto:true,idUsuario:id29U_,estado:est29U_,mensaje:"Usuario actualizado."}; }
    ...
    if (operation === "guardarUsuarioAdminMotor") { const x=args[0] || {}; if(!x.idUsuario){const invited=await client.functions.invoke("invite-user",{body:{usuario:x,redirectTo:window.location.origin+window.location.pathname}});if(invited.error){let detail="";try{detail=await invited.error.context.json();}catch(_){ }throw new Error(detail&&detail.error || "No se encontró la función segura de invitaciones en Supabase. Despliega invite-user desde Edge Functions antes de crear usuarios.");}if(!invited.data || invited.data.correcto===false)throw new Error(invited.data && invited.data.error || "No fue posible enviar la invitación.");return invited.data;} if(String(x.nuevaContrasena||"").trim()){const reset=await client.functions.invoke("invite-user",{body:{accion:"CAMBIAR_CONTRASENA",idUsuario:x.idUsuario,nuevaContrasena:x.nuevaContrasena}});if(reset.error||!reset.data||reset.data.correcto===false)throw new Error(reset.data&&reset.data.error||"No fue posible cambiar la contraseña.");} const payloadUsr29U_={correo:String(x.correo || "").trim().toLowerCase(),nombre:x.nombre,telefono:x.telefono || null,rol:x.rol,id_proveedor:x.idProveedor || null,id_oficina:x.idOficina || null,id_grupo:x.idGrupo || null,estado:x.estado || "ACTIVO",tipo_documento:x.tipoDocumento || null,numero_documento:x.numeroDocumento || null,id_usuario_actualizacion:user.id_usuario}; if(String(x.idEmpresa||"").trim()) payloadUsr29U_.id_empresa=String(x.idEmpresa).trim(); var saved=await client.from("seg_usuarios").update(payloadUsr29U_).eq("id_usuario",x.idUsuario); if(saved.error&&String((saved.error&&saved.error.message)||"").indexOf("id_empresa")!==-1){try{delete payloadUsr29U_.id_empresa;}catch(_){} saved=await client.from("seg_usuarios").update(payloadUsr29U_).eq("id_usuario",x.idUsuario);} if(saved.error) throw saved.error; return {correcto:true,idUsuario:x.idUsuario}; }
    ...
    if (operation === "obtenerPlantillaProveedoresModulo") return file("plantilla_proveedores.csv",["RAZON_SOCIAL","NOMBRE_COMERCIAL","CODIGO_SAP","RUC","DESCRIPCION"],[]);
    ...
    if (operation === "guardarMaterialPrecioModulo") { const x=args[0] || {}; const id=x.idMaterial || "MAT-"+Date.now(); var chainWarnProv364_ = []; try { var canalMat364_ = String(x.idCanal || x.canal || "").trim(); var chain364_ = await ensureProveedorChain_(client, user, { proveedor: (x.proveedor || x.nombreProveedor || ""), oficina: (x.oficina || x.nombreOficina || x.oficinaTexto || ""), grupo: (x.grupo || x.nombreGrupo || x.grupoTexto || ""), idProveedor: (x.idProveedor || ""), idOficina: (x.idOficina || ""), idGrupo: (x.idGrupo || ""), idCanal: canalMat364_ }, canalMat364_); if (chain364_ && chain364_.advertencias) chainWarnProv364_ = chain364_.advertencias; } catch (chainError364_) { try { console.warn("[SGT360] Auto-creación proveedor (material):", chainError364_); } catch (_) {} chainWarnProv364_.push("No se pudo completar la cadena de proveedor: " + String((chainError364_ && chainError364_.message) || chainError364_)); } let brandId=String(x.idMarca || "").trim(); if(brandId){const brands=await query("mae_marcas");const found=brands.find(function(b){return b.id_marca===brandId || String(b.nombre).toLowerCase()===brandId.toLowerCase();});if(found)brandId=found.id_marca;else{brandId="MAR-"+Date.now();const brand=await client.from("mae_marcas").insert({id_marca:brandId,codigo_marca:brandId,nombre:String(x.idMarca).trim(),estado:"ACTIVO",id_usuario_actualizacion:user.id_usuario});if(brand.error)throw brand.error;}} const saved=await client.from("mae_materiales").upsert({id_material:id,codigo_material:x.codigoMaterial || id,codigo_sap:x.codigoHana || x.codigoSap || null,codigo_hana:x.codigoHana || x.codigoSap || null,id_producto:x.idProducto || null,id_tipo_material:x.idTipoMaterial || null,id_subtipo_material:x.idSubtipoMaterial || null,id_marca:brandId || null,nombre_material:x.nombreMaterial || x.descripcionMaterial || "Material",descripcion_material:x.descripcionMaterial || null,proveedor:x.proveedor || null,incluye_conexion:x.incluyeConexion || x.incluye_conexion || null,producto_principal:x.productoPrincipal || x.producto_principal || null,combo:x.combo || null,comentarios:x.comentarios || null,unidad_medida:x.unidadMedida || "UND",es_gasodomestico:!!x.esGasodomestico,estado:x.estado || "ACTIVO",id_usuario_actualizacion:user.id_usuario}); if(saved.error) throw saved.error; return {correcto:true,idMaterial:id,mensaje:"Material guardado correctamente." + (chainWarnProv364_.length ? " " + chainWarnProv364_.join(" ") : ""),advertencias:chainWarnProv364_}; }
    if (operation === "asegurarTaxonomiaMaterialesModulo") {
      const combo29X_ = args[0] || {};
      const items29X_ = Array.isArray(combo29X_.items) ? combo29X_.items : [];
      const fold29X_ = function(s) { try { return String(s || "").normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase().trim(); } catch (_) { return String(s || "").toLowerCase().trim(); } };
      const slug29X_ = function(s, fb) { try { var b = String(s || "").normalize("NFD").replace(/[\u0300-\u036f]/g, "").toUpperCase().replace(/[^A-Z0-9]+/g, "_").replace(/^_+|_+$/g, "").slice(0, 40); return b || fb; } catch (_) { return fb; } };
      const seen29X_ = {}, distintos29X_ = [];
      items29X_.forEach(function(r) {
        r = r || {};
        const t29X_ = String(r.tipo || "").trim(), s29X_ = String(r.subtipo || "").trim(), p29X_ = String(r.producto || r.productoPrincipal || "").trim();
        if (!t29X_ && !s29X_) return;
        const k29X_ = (p29X_ + "|" + t29X_ + "|" + s29X_).toUpperCase();
        if (!seen29X_[k29X_]) { seen29X_[k29X_] = true; distintos29X_.push({ producto: p29X_, tipo: t29X_, subtipo: s29X_ }); }
      });
      if (!distintos29X_.length) return { correcto: true, tiposCreados: [], subtiposCreados: [], advertencias: [] };
      const tiposQ29X_ = await client.from("mae_tipos_material").select("id_tipo_material,codigo_tipo,nombre");
      if (tiposQ29X_.error) throw tiposQ29X_.error;
      const subtiposQ29X_ = await client.from("mae_subtipos_material").select("id_subtipo_material,id_tipo_material,id_producto,codigo_subtipo,nombre");
      if (subtiposQ29X_.error) throw subtiposQ29X_.error;
      const prodsQ29X_ = await client.from("mae_productos_principales").select("id_producto,nombre");
      if (prodsQ29X_.error) throw prodsQ29X_.error;
      const tipos29X_ = tiposQ29X_.data || [], subtipos29X_ = subtiposQ29X_.data || [], prods29X_ = prodsQ29X_.data || [];
      const creadosT29X_ = [], creadosS29X_ = [], avisos29X_ = [];
      let seq29X_ = 0;
      for (let ci29X_ = 0; ci29X_ < distintos29X_.length; ci29X_++) {
        const combo = distintos29X_[ci29X_];
        let tipoRow29X_ = null;
        if (combo.tipo) {
          tipoRow29X_ = tipos29X_.filter(function(t) { return fold29X_(t.nombre) === fold29X_(combo.tipo); })[0] || null;
          if (!tipoRow29X_) {
            let slugT29X_ = slug29X_(combo.tipo, "TIP-AUTO");
            let nT29X_ = 0;
            while (tipos29X_.some(function(t) { return String(t.codigo_tipo || "").toUpperCase() === slugT29X_ + (nT29X_ ? "-" + nT29X_ : ""); })) nT29X_++;
            if (nT29X_) slugT29X_ += "-" + nT29X_;
            const nuevoT29X_ = { id_tipo_material: "TIP-AUTO-" + Date.now() + "-" + (seq29X_++), codigo_tipo: slugT29X_, nombre: combo.tipo, descripcion: null, estado: "ACTIVO", id_usuario_actualizacion: user.id_usuario };
            const insT29X_ = await client.from("mae_tipos_material").insert(nuevoT29X_);
            if (insT29X_.error) { avisos29X_.push("No se pudo crear el tipo " + combo.tipo + ": " + insT29X_.error.message); continue; }
            tipos29X_.push(nuevoT29X_);
            tipoRow29X_ = nuevoT29X_;
            creadosT29X_.push(combo.tipo);
          }
        }
        let prodRow29X_ = null;
        if (combo.producto) prodRow29X_ = prods29X_.filter(function(p) { return fold29X_(p.nombre) === fold29X_(combo.producto); })[0] || null;
        if (combo.subtipo) {
          if (!tipoRow29X_) { avisos29X_.push("Subtipo sin tipo (" + combo.subtipo + "): indica el tipo para crearlo."); continue; }
          const exS29X_ = subtipos29X_.filter(function(s) { return String(s.id_tipo_material || "") === String(tipoRow29X_.id_tipo_material || "") && fold29X_(s.nombre) === fold29X_(combo.subtipo); })[0] || null;
          if (!exS29X_) {
            const nuevoS29X_ = { id_subtipo_material: "SUB-AUTO-" + Date.now() + "-" + (seq29X_++), id_tipo_material: tipoRow29X_.id_tipo_material, id_producto: prodRow29X_ ? prodRow29X_.id_producto : null, codigo_subtipo: slug29X_(combo.subtipo, "SUB-AUTO"), nombre: combo.subtipo, descripcion: null, estado: "ACTIVO", id_usuario_actualizacion: user.id_usuario };
            const insS29X_ = await client.from("mae_subtipos_material").insert(nuevoS29X_);
            if (insS29X_.error) { avisos29X_.push("No se pudo crear el subtipo " + combo.subtipo + ": " + insS29X_.error.message); continue; }
            subtipos29X_.push(nuevoS29X_);
            creadosS29X_.push(combo.subtipo);
          }
        }
      }
      return { correcto: true, tiposCreados: creadosT29X_, subtiposCreados: creadosS29X_, advertencias: avisos29X_ };
    }
    if (operation === "listarProveedoresModulo") { const rows = await query("mae_proveedores"); const records = rows.map(function(x) { return { idProveedor:x.id_proveedor,razonSocial:x.razon_social,nombreComercial:x.nombre_comercial,codigoSap:x.codigo_sap,ruc:x.ruc,descripcion:x.descripcion,idsOficina:String(x.codigo_canales_venta||"").split("|").filter(Boolean),idsGrupo:String(x.codigo_grupos_vendedores||"").split("|").filter(Boolean),alcanceCatalogo:x.alcance_catalogo,idCanalOrigen:x.id_canal_origen||"",estado:x.estado }; }); return normalizeModuleResponse(operation, { proveedores: records }); }
    ...
    if (operation === "guardarListaOficialPrecioModulo") { const L=args[0]||{}; var idCanalL_=String(L.idCanal||L.canal||L.id_canal||"").trim().toUpperCase(); try { if(idCanalL_==="IA") idCanalL_="CANAL-IA"; else if(idCanalL_==="ALO") idCanalL_="CANAL-ALO"; } catch(_){} var idProvL=String(L.idProveedor||"").trim(); var provTextoL_=String(L.proveedor||L.nombreProveedor||L.razonSocial||"").trim(); try { if(!idProvL&&provTextoL_){var chainL0_=await ensureProveedorChain_(client, user, {proveedor:provTextoL_,idCanal:idCanalL_}, idCanalL_); if(chainL0_&&chainL0_.idProveedor) idProvL=String(chainL0_.idProveedor);} else if(idProvL){var existeL0_=await client.from("mae_proveedores").select("id_proveedor").eq("id_proveedor",idProvL).maybeSingle(); if(!existeL0_.error&&!existeL0_.data){if(provTextoL_){var chainL1_=await ensureProveedorChain_(client, user, {proveedor:provTextoL_,idCanal:idCanalL_}, idCanalL_); if(chainL1_&&chainL1_.idProveedor) idProvL=String(chainL1_.idProveedor);}else{try{var autoProvL_={id_proveedor:idProvL,razon_social:idProvL,nombre_comercial:idProvL,nombre:idProvL,alcance_catalogo:"TOTAL",estado:"ACTIVO",id_usuario_actualizacion:user.id_usuario}; if(idCanalL_) autoProvL_.id_canal_origen=idCanalL_; var autoInsL_=await client.from("mae_proveedores").insert(autoProvL_); if(autoInsL_.error&&String((autoInsL_.error&&autoInsL_.error.message)||"").indexOf("id_canal_origen")!==-1){try{delete autoProvL_.id_canal_origen;}catch(_){} autoInsL_=await client.from("mae_proveedores").insert(autoProvL_);} }catch(_){}}}} } catch(autoErrL_){try{console.warn("[SGT360] Auto-creacion proveedor (lista):", autoErrL_);}catch(_){}} /* Lista de canal (idCanalL_ presente): proveedor en cabecera es OPCIONAL (va en cada detalle). Solo exigir proveedor si NO hay canal. */ if(!idCanalL_&&!idProvL) throw new Error("Selecciona el proveedor. Todo precio debe pertenecer a un proveedor; General solo significa sin oficina ni grupo."); if(idProvL){ const provL=await client.from("mae_proveedores").select("id_proveedor,estado").eq("id_proveedor",idProvL).maybeSingle(); if(provL.error) throw provL.error; if(!provL.data) throw new Error("El proveedor seleccionado no existe o está inactivo."); } const ofL=String(L.idOficina||"").trim(); let grL=String(L.idGrupo||"").trim(); if(grL && !ofL) throw new Error("Para usar grupo debes seleccionar una oficina de ventas."); if(!ofL) grL=""; if(grL){ const grRow=await client.from("mae_grupos").select("id_grupo,id_oficina,estado").eq("id_grupo",grL).maybeSingle(); if(grRow.error) throw grRow.error; if(!grRow.data) throw new Error("El grupo seleccionado no existe."); if(String(grRow.data.id_oficina||"").trim() && String(grRow.data.id_oficina).trim()!==ofL) throw new Error("El grupo no pertenece a la oficina indicada."); } const negL=String(L.idNegocio||"").trim(); if(negL){ const negRow=await client.from("mae_negocios").select("id_negocio").eq("id_negocio",negL).maybeSingle(); if(negRow.error) throw negRow.error; if(!negRow.data) throw new Error("El negocio indicado no existe."); } const nowL=new Date(); const defIniL=nowL.toISOString().slice(0,8)+"01"; const defFinL=new Date(nowL.getFullYear(),nowL.getMonth()+1,0).toISOString().slice(0,10); const iniL=String(L.fechaInicio||"").trim().slice(0,10)||defIniL; const finL=String(L.fechaFin||"").trim().slice(0,10)||defFinL; if(finL<iniL) throw new Error("La fecha fin no puede ser menor que la fecha inicio."); const codigoL=String(L.codigoLista||"").trim(); const nombreL=String(L.nombre||"").trim()||"Lista de precios"; const wantedL=String(L.idListaPrecio||"").trim(); let rowL=null; if(wantedL){ const prevL=await client.from("pre_listas_precios").select("*").eq("id_lista_precio",wantedL).maybeSingle(); if(prevL.error) throw prevL.error; rowL=prevL.data||null; } const idL=rowL?rowL.id_lista_precio:(wantedL||("LPR-"+Date.now()+"-"+Math.floor(Math.random()*1000))); const payloadL={id_lista_precio:idL,codigo_lista:codigoL||(rowL?rowL.codigo_lista:idL),nombre:nombreL,id_proveedor:idProvL||null,id_negocio:negL||null,id_oficina:ofL||null,id_grupo:grL||null,responsable_venta:String(L.responsableVenta||"").trim()||null,fecha_inicio:iniL,fecha_fin:finL||null,moneda:String(L.moneda||"PEN").trim()||"PEN",estado:String(L.estado||"ACTIVA").trim()||"ACTIVA",es_catalogo:(L.es_catalogo===true||L.esCatalogo===true)||((rowL&&rowL.es_catalogo)===true),id_usuario_actualizacion:user.id_usuario};try{if(idCanalL_)payloadL.id_canal=idCanalL_;}catch(_){}var savedListL=await client.from("pre_listas_precios").upsert(payloadL,{onConflict:"id_lista_precio"});if(savedListL.error){var msgListL_=String((savedListL.error&&savedListL.error.message)||"");var retryListL_=false;if(msgListL_.indexOf("es_catalogo")!==-1){try{delete payloadL.es_catalogo;}catch(_){}retryListL_=true;}if(msgListL_.indexOf("id_canal")!==-1){try{delete payloadL.id_canal;}catch(_){}retryListL_=true;}if(retryListL_)savedListL=await client.from("pre_listas_precios").upsert(payloadL,{onConflict:"id_lista_precio"});} const savedL=savedListL; if(savedL.error) throw new Error("No se pudo guardar la lista: "+savedL.error.message); return {correcto:true,idListaPrecio:idL,creado:!rowL,mensaje:!rowL?"Lista oficial creada.":"Lista oficial actualizada."}; }
    if (operation === "guardarDetalleListaPrecioModulo") { const D=args[0]||{}; const idListaD=String(D.idListaPrecio||"").trim(); if(!idListaD) throw new Error("Indica la lista de precios del detalle."); const listaD=await client.from("pre_listas_precios").select("id_lista_precio").eq("id_lista_precio",idListaD).maybeSingle(); if(listaD.error) throw listaD.error; if(!listaD.data) throw new Error("La lista de precios no existe."); const idMatD=String(D.idMaterial||"").trim(); if(!idMatD) throw new Error("Indica el material del precio."); const matD=await client.from("mae_materiales").select("id_material,estado").eq("id_material",idMatD).maybeSingle(); if(matD.error) throw matD.error; if(!matD.data) throw new Error("El material no existe."); const precioD=Number(D.precioBase); if(!Number.isFinite(precioD)||precioD<0) throw new Error("Precio base inválido."); let feeD=null; if(D.fee!==null&&D.fee!==undefined&&String(D.fee).trim()!==""){ feeD=Number(D.fee); if(!Number.isFinite(feeD)||feeD<0||feeD>100) throw new Error("El fee debe ser un porcentaje entre 0 y 100."); feeD=Math.round(feeD*1000)/1000; } let idDetD=String(D.idDetallePrecio||"").trim(); let creadoD=true; if(!idDetD){ const prevD=await client.from("pre_lista_precio_detalle").select("id_detalle_precio").eq("id_lista_precio",idListaD).eq("id_material",idMatD).maybeSingle(); if(prevD.error) throw prevD.error; if(prevD.data){ idDetD=prevD.data.id_detalle_precio; creadoD=false; } } if(!idDetD) idDetD="DPR-"+Date.now()+"-"+Math.floor(Math.random()*10000); var idProvD_=String(D.idProveedor||D.id_proveedor||"").trim()||null; var provTextoD_=String(D.proveedorTexto||D.proveedor||"").trim()||null; try { if(idProvD_) { const existeD_=await client.from("mae_proveedores").select("id_proveedor").eq("id_proveedor",idProvD_).maybeSingle(); if(!existeD_.error&&!existeD_.data) { var chainD_=await ensureProveedorChain_(client,user,{proveedor:idProvD_,idCanal:""},null); if(chainD_&&chainD_.idProveedor) idProvD_=String(chainD_.idProveedor); } } else if(provTextoD_) { var chainD2_=await ensureProveedorChain_(client,user,{proveedor:provTextoD_,idCanal:""},null); if(chainD2_&&chainD2_.idProveedor) idProvD_=String(chainD2_.idProveedor); } } catch(provDErr_){try{console.warn("[SGT360] Auto-creacion proveedor (detalle):", provDErr_);}catch(_){}} const payloadD={id_detalle_precio:idDetD,codigo_precio:idDetD,id_lista_precio:idListaD,id_material:idMatD,precio_base:precioD,moneda:String(D.moneda||"PEN").trim()||"PEN",fee:feeD,responsable_venta:String(D.responsableVenta||"").trim()||null,tiene_combo:false,detalle_combo:null,estado:String(D.estado||"ACTIVO").trim()||"ACTIVO",id_usuario_actualizacion:user.id_usuario}; if(idProvD_) payloadD.id_proveedor=idProvD_; var savedD=await client.from("pre_lista_precio_detalle").upsert(payloadD,{onConflict:"id_detalle_precio"}); if(savedD.error&&String((savedD.error&&savedD.error.message)||"").indexOf("id_proveedor")!==-1){ try{delete payloadD.id_proveedor;}catch(_){} savedD=await client.from("pre_lista_precio_detalle").upsert(payloadD,{onConflict:"id_detalle_precio"}); } if(savedD.error) throw new Error("No se pudo guardar el detalle: "+savedD.error.message); return {correcto:true,idDetallePrecio:idDetD,idListaPrecio:idListaD,creado:creadoD,mensaje:creadoD?"Detalle creado.":"Detalle actualizado."}; }
    ...
    if (operation === "exportarPreciosOficialesModulo") { const result=await executeSupabaseOperation(client,"listarListasOficialesPreciosModulo",args,user); var mpIsProv=String(user && user.rol || "").toUpperCase()==="PROVEEDOR"; var mpHeads=mpIsProv?["PROVEEDOR","NEGOCIO","CODIGO_MATERIAL","MATERIAL","PRECIO","MONEDA","INICIO","FIN"]:["PROVEEDOR","NEGOCIO","CODIGO_MATERIAL","MATERIAL","PRECIO","FEE","MONEDA","INICIO","FIN"]; return file("precios.csv",mpHeads,result.registros.map(function(x){var base=[x.proveedor,x.negocio,x.codigoMaterial,x.nombreCortoMaterial,x.precioBase]; if(!mpIsProv)base.push(x.fee==null?"":x.fee); return base.concat([x.moneda,x.fechaInicio,x.fechaFin]);})); }
    ...
    if (operation === "actualizarFeeMaterialPrecioModulo") { var feeInput_=args[0]||{}; var idDetFee_=String(feeInput_.idDetallePrecio||"").trim(); if(!idDetFee_) throw new Error("Indica el detalle de precio."); var feeNum_=Number(feeInput_.fee); if(!Number.isFinite(feeNum_)||feeNum_<0||feeNum_>100) throw new Error("El fee debe ser un porcentaje entre 0 y 100."); var rolFee_=String(user.rol||"").toUpperCase(); if(rolFee_!=="SUPERADMIN"){var permFeeQ_=await client.from("seg_permisos").select("id_sujeto,recurso,permitido,alcance").eq("estado","ACTIVO").eq("modulo","MATERIALES_PRECIOS").eq("recurso","EDITAR_FEE").in("id_sujeto",[user.id_usuario,rolFee_]); if(permFeeQ_.error) throw permFeeQ_.error; var rowsFee_=(permFeeQ_.data||[]).filter(function(r){return r.permitido===true;}); var mineFee_=rowsFee_.filter(function(r){return String(r.id_sujeto||"")===String(user.id_usuario||"");})[0]||rowsFee_.filter(function(r){return String(r.id_sujeto||"").toUpperCase()===rolFee_;})[0]||null; if(!mineFee_) throw new Error("No tienes permiso para editar el fee (MATERIALES_PRECIOS/EDITAR_FEE)."); var alcFee_=String(mineFee_.alcance||"").toUpperCase(); if(alcFee_==="GLOBAL"||alcFee_==="TOTAL"){}else if(alcFee_==="PROVEEDOR"||alcFee_==="ASIGNADOS"){var provFeeU_=String(user.id_proveedor||user.idProveedor||"").trim(); if(!provFeeU_) throw new Error("No tienes proveedor asignado para editar el fee."); var detFeeQ_=await client.from("pre_lista_precio_detalle").select("id_detalle_precio,id_lista_precio").eq("id_detalle_precio",idDetFee_).maybeSingle(); if(detFeeQ_.error) throw detFeeQ_.error; if(!detFeeQ_.data) throw new Error("No se encontro el detalle de precio."); var listFeeQ_=await client.from("pre_listas_precios").select("id_lista_precio,id_proveedor").eq("id_lista_precio",detFeeQ_.data.id_lista_precio).maybeSingle(); if(listFeeQ_.error) throw listFeeQ_.error; if(!listFeeQ_.data||String(listFeeQ_.data.id_proveedor||"").trim()!==provFeeU_) throw new Error("Solo una cuenta del proveedor de esta lista puede editar el fee (alcance "+alcFee_+").");}else{throw new Error("No tienes acceso para editar el fee con tu alcance actual ("+alcFee_+").");}} var updFee_=await client.from("pre_lista_precio_detalle").update({fee:feeNum_,id_usuario_actualizacion:user.id_usuario}).eq("id_detalle_precio",idDetFee_); if(updFee_.error) throw updFee_.error; return {correcto:true,idDetallePrecio:idDetFee_,fee:feeNum_,mensaje:"Fee actualizado correctamente."}; }
    if (operation === "guardarMatrizPermisosUsuarioMotor") { var rolMatU_=String(user.rol||"").toUpperCase(); if(["SUPERADMIN","ADMIN"].indexOf(rolMatU_)===-1) throw new Error("Solo un administrador puede asignar permisos a usuarios."); var payloadMU_=args[0]||{}; var idUsuMU_=String(payloadMU_.idUsuario||"").trim(); if(!idUsuMU_) throw new Error("Indica el usuario."); var changesMU_=payloadMU_.permisos||[]; var permissionIdMU_=function(value){var hash=5381,text=String(value||"");for(var j=0;j<text.length;j+=1)hash=((hash<<5)+hash)^text.charCodeAt(j);return "PERM-"+(hash>>>0).toString(36);}; for(var iMU_=0;iMU_<changesMU_.length;iMU_+=1){var xMU_=changesMU_[iMU_]; if(!xMU_.modulo||!xMU_.recurso) throw new Error("Cada permiso debe indicar modulo y recurso."); var rowMU_={id_permiso:permissionIdMU_([idUsuMU_,xMU_.modulo,xMU_.recurso].join("|")),tipo_sujeto:"USUARIO",id_sujeto:idUsuMU_,modulo:xMU_.modulo,recurso:xMU_.recurso,permitido:!!xMU_.permitido,alcance:xMU_.alcance||"PROPIO",estado:"ACTIVO"}; var savedMU_=await client.from("seg_permisos").upsert(rowMU_); if(savedMU_.error) throw savedMU_.error;} return {correcto:true,idUsuario:idUsuMU_,mensaje:"Permisos del usuario actualizados."}; }
    if (operation === "obtenerMatrizPermisosUsuarioMotor") { var idUsuMO_=String((args[0]&&args[0].idUsuario)||args[0]||"").trim(); if(!idUsuMO_) throw new Error("Indica el usuario."); var rowsMO_=await client.from("seg_permisos").select("modulo,recurso,permitido,alcance,estado").eq("tipo_sujeto","USUARIO").eq("id_sujeto",idUsuMO_).eq("estado","ACTIVO"); if(rowsMO_.error) throw rowsMO_.error; var permsMO_=(rowsMO_.data||[]).map(function(r){return {modulo:r.modulo,recurso:r.recurso,permitido:r.permitido===true,alcance:r.alcance||"PROPIO"};}); return {correcto:true,idUsuario:idUsuMO_,permisos:permsMO_}; }
    if (operation === "listarCanalesAdminMotor") { try { var rowsCA_=await query("ven_canales"); var recCA_=rowsCA_.map(function(r){return {idCanal:r.id_canal,codigo:r.codigo,nombre:r.nombre,descripcion:r.descripcion,estado:r.estado,id_canal:r.id_canal};}); return {correcto:true,registros:recCA_,canales:recCA_}; } catch(e){ throw new Error("No se pudieron listar los canales (ven_canales): "+String((e&&e.message)||e)); } }
    if (operation === "guardarCanalAdminMotor") { var dCA_=args[0]||{}; var codCA_=String(dCA_.codigo||"").trim().toUpperCase(); if(!codCA_) throw new Error("Ingresa el codigo del canal."); var idCA_=String(dCA_.idCanal||dCA_.id_canal||"").trim(); if(!idCA_){ try { var byCodCA_=await client.from("ven_canales").select("id_canal").eq("codigo",codCA_).maybeSingle(); if(!byCodCA_.error&&byCodCA_.data&&byCodCA_.data.id_canal) idCA_=String(byCodCA_.data.id_canal).trim(); } catch(_){} } if(!idCA_) idCA_="CANAL-"+codCA_; var rowCA_={id_canal:idCA_,codigo:codCA_,nombre:String(dCA_.nombre||codCA_).trim()||codCA_,descripcion:String(dCA_.descripcion||"")||null,estado:String(dCA_.estado||"ACTIVO").trim()||"ACTIVO"}; try { var svCA_=await client.from("ven_canales").upsert(rowCA_,{onConflict:"id_canal"}); if(svCA_.error) throw svCA_.error; } catch(e){ throw new Error("No se pudo guardar el canal (ven_canales): "+String((e&&e.message)||e)); } return {correcto:true,idCanal:idCA_,codigo:codCA_,mensaje:"Canal guardado."}; }
    if (operation === "listarEmpresasVendedorasAdminMotor") { try { var allEV_=await Promise.all([query("ven_empresas"),query("ven_canales"),query("mae_proveedores")]); var recEV_=allEV_[0].map(function(r){ var chEV2_=allEV_[1].filter(function(c){return String(c.id_canal||"")===String(r.id_canal||"");})[0]||{}; var pvEV2_=allEV_[2].filter(function(p){return String(p.id_proveedor||"")===String(r.id_proveedor||"");})[0]||{}; return {idEmpresa:r.id_empresa,nombre:r.nombre,tipo:r.tipo,idCanal:r.id_canal,canal:r.id_canal,codigoCanal:chEV2_.codigo||"",nombreCanal:chEV2_.nombre||chEV2_.codigo||r.id_canal||"",idProveedor:r.id_proveedor,nombreProveedor:pvEV2_.nombre_comercial||pvEV2_.razon_social||"",estado:r.estado,id_empresa:r.id_empresa,id_canal:r.id_canal};}); return {correcto:true,registros:recEV_,empresas:recEV_}; } catch(e){ throw new Error("No se pudieron listar las empresas (ven_empresas): "+String((e&&e.message)||e)); } }
    if (operation === "guardarEmpresaVendedoraAdminMotor") { var dEV_=args[0]||{}; var nomEV_=String(dEV_.nombre||"").trim(); if(!nomEV_) throw new Error("Ingresa el nombre de la empresa."); var tipoEV_=String(dEV_.tipo||"").trim().toUpperCase(); if(tipoEV_!=="ALO"&&tipoEV_!=="IA") throw new Error("El tipo debe ser ALO o IA."); var canalInEV_=String(dEV_.idCanal||dEV_.canal||dEV_.id_canal||"").trim().toUpperCase(); var idCanalEV_=canalInEV_; if(idCanalEV_==="IA")idCanalEV_="CANAL-IA"; else if(idCanalEV_==="ALO")idCanalEV_="CANAL-ALO"; if(idCanalEV_.indexOf("CANAL-")!==0){ try { var chEV_=await client.from("ven_canales").select("id_canal,codigo").or("codigo.eq."+idCanalEV_+",id_canal.eq."+idCanalEV_).maybeSingle(); if(!chEV_.error&&chEV_.data&&chEV_.data.id_canal) idCanalEV_=String(chEV_.data.id_canal).trim(); } catch(_){} } if(idCanalEV_){ var vexEV_=await client.from("ven_canales").select("id_canal").eq("id_canal",idCanalEV_).maybeSingle(); if(vexEV_.error) throw new Error("No se pudo verificar el canal: "+vexEV_.error.message); if(!vexEV_.data) throw new Error("El canal indicado no existe. Registralo primero en la tarjeta Canales."); } if(!idCanalEV_) throw new Error("Selecciona el canal de la empresa."); var idProvEV_=String(dEV_.idProveedor||dEV_.id_proveedor||"").trim()||null; if(idProvEV_){ var pvEV_=await client.from("mae_proveedores").select("id_proveedor").eq("id_proveedor",idProvEV_).maybeSingle(); if(pvEV_.error) throw new Error("No se pudo verificar el proveedor: "+pvEV_.error.message); if(!pvEV_.data) throw new Error("El proveedor vinculado no existe."); } var idEV_=String(dEV_.idEmpresa||dEV_.id_empresa||"").trim(); if(!idEV_) idEV_="EMP-"+Date.now().toString(36).toUpperCase()+"-"+Math.floor(Math.random()*1296).toString(36).toUpperCase(); var rowEV_={id_empresa:idEV_,nombre:nomEV_,tipo:tipoEV_,id_canal:idCanalEV_,id_proveedor:idProvEV_,estado:String(dEV_.estado||"ACTIVO").trim()||"ACTIVO"}; try { var svEV_=await client.from("ven_empresas").upsert(rowEV_,{onConflict:"id_empresa"}); if(svEV_.error) throw svEV_.error; } catch(e){ throw new Error("No se pudo guardar la empresa (ven_empresas): "+String((e&&e.message)||e)); } return {correcto:true,idEmpresa:idEV_,mensaje:"Empresa guardada."}; }
    if (operation === "listarMaterialesFeePorListaProveedorModulo") { var fMF_=args[0]||{}; if(typeof fMF_==="string") fMF_={idListaPrecio:fMF_}; var idListaMF_=String(fMF_.idListaPrecio||fMF_.idLista||"").trim(); if(!idListaMF_) throw new Error("Indica la lista de precios."); var baseMF_=await executeSupabaseOperation(client,"listarListasOficialesPreciosModulo",[{}],user); var rowsMF_=(baseMF_.registros||[]).filter(function(r){ if(String(r.idListaPrecio||"")!==idListaMF_) return false; var wantProvMF_=String(fMF_.idProveedor||"").trim(); if(wantProvMF_&&String(r.idProveedor||"")!==wantProvMF_) return false; return true; }); return {correcto:true,idListaPrecio:idListaMF_,registros:rowsMF_,paginacion:{pagina:1,totalPaginas:1,total:rowsMF_.length}}; }
    return undefined;
  }
