const MP_STATE = {
  ...
  function inicializarArbolTipificacionMaterialForm_(form, opts, materialData) {
    ...
    [productoSelect, tipoSelect, subtipoSelect, form.elements["idNegocio"]]
      .filter(Boolean)
      .forEach(makeMpSelectSearchable_);
  }

  function makeMpSelectSearchable_(select) {
    if (!select || select.dataset.searchReady === "1") return;
    select.dataset.searchReady = "1";
    const search = document.createElement("input");
    search.type = "search";
    search.className = "mp-select-search";
    search.placeholder = "Escribe para buscar…";
    search.setAttribute("aria-label", "Buscar " + (select.name || "opción"));
    select.parentNode.insertBefore(search, select);
    search.addEventListener("input", function() {
      const term = String(search.value || "").toLowerCase().trim();
      Array.from(select.options).forEach(function(option, index) {
        option.hidden = index > 0 && Boolean(term) && option.text.toLowerCase().indexOf(term) === -1;
      });
    });
  }
