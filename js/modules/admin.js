const ADMIN_STATE = {
  ...
  function renderPermissionMatrix() {
    ...
    region.innerHTML = (matrix.modulos || []).map(function(module) {
      ...
      const allEnabled = resources.length > 0 && resources.every(function(resource) { return resource.permitido === true; });
      const sharedScope = (resources[0] && resources[0].alcance) || "PROPIO";
      const scopes = matrix.alcancesDisponibles || ["PROPIO", "PROVEEDOR", "GRUPO", "ASIGNADOS", "GLOBAL"];
      return '<section class="permission-module" data-permission-module-section="' + escapeHtml(module.codigo) + '"><header class="permission-module-header"><span class="material-symbols-rounded">dataset</span><strong>' + escapeHtml(module.codigo) + '</strong><span style="margin-left:auto;display:flex;align-items:center;gap:10px"><label class="switch" title="Habilitar o deshabilitar todas las funciones del módulo"><input type="checkbox" data-module-enabled ' + (allEnabled ? "checked" : "") + '><span class="switch-track"></span></label><select data-module-scope title="Aplicar alcance a todas las funciones">' + scopes.map(function(scope) { return '<option value="' + scope + '" ' + (sharedScope === scope ? "selected" : "") + '>' + scope + '</option>'; }).join("") + '</select></span></header><div class="permission-list">' +
    ...
    region.querySelectorAll("[data-permission-enabled]").forEach(function(input) {
      input.addEventListener("change", function() {
        ...
        const section = row.closest("[data-permission-module-section]");
        if (section) section.querySelector("[data-module-enabled]").checked =
          Array.from(section.querySelectorAll("[data-permission-enabled]")).every(function(item) {
            return item.checked;
          });
      });
    });
    region.querySelectorAll("[data-module-enabled]").forEach(function(input) {
      input.addEventListener("change", function() {
        const section = input.closest("[data-permission-module-section]");
        section.querySelectorAll(".permission-row").forEach(function(row) {
          row.querySelector("[data-permission-enabled]").checked = input.checked;
          row.querySelector("[data-permission-scope]").disabled = !input.checked;
        });
      });
    });
    region.querySelectorAll("[data-module-scope]").forEach(function(select) {
      select.addEventListener("change", function() {
        const section = select.closest("[data-permission-module-section]");
        section.querySelectorAll("[data-permission-scope]").forEach(function(scope) {
  ...
  function refreshUserAssignmentOptions(selectedOffice, selectedGroup) {
    ...
    const isCommercialRole = /VENDEDOR|COORDINADOR/.test(roleCode);
  ...
  function updateUserAssignmentRequirements() {
    ...
    const isCommercialRole = /VENDEDOR|COORDINADOR/.test(String(roleSelect.value || "").toUpperCase());
