const APP_STORAGE = Object.freeze({
    ...
    google.script.run
      .withSuccessHandler(function(result) {
        if (!result || result.correcto !== true || result.autenticado !== true) {
          clearLocalSession();
          showLogin(result && result.mensaje ? result.mensaje : "La sesión no es válida.", true);
          return;
        }

        APP_STATE.session = result;
        initializeProtectedApplication();
      })
      .withFailureHandler(function(error) {
        clearLocalSession();
        showLogin(errorMessage(error, "No fue posible validar la sesión."), true);
      })
  ...
  function applyInitialLoaderBranding() {
    ...
    const source = String(config.logoDataUrl || "").trim();
